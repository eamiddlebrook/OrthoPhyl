...
class AssemblyRouter:
    ...
    def __init__(
        ...
        Args:
            ...
            similarity_threshold: ANI threshold for considering genomes "related"
        """
    ...
    def _find_and_download_related_genomes(
        ...
        # Fallback: create placeholder or use local genomes
...
def create_example_database(output_dir: Path):
    ...
    This is a helper function to set up the expected database format.
    """
    ...
    with open(taxonomy_map, 'w') as f:
        f.write("# Taxonomy database mapping\n")
        f.write("# Format: taxonomy<TAB>level<TAB>genome_ids (comma-separated)\n")
        f.write("taxonomy\tlevel\tgenomes\n")
        f.write("Escherichia\tgenus\tGCF_000005845.2,GCF_000008865.2,GCF_000482265.1\n")
        f.write("Escherichia coli\tspecies\tGCF_000005845.2,GCF_000008865.2,GCF_000482265.1,GCF_000742135.1\n")
    ...
    metadata = {
        ...
        "version": "1.0",
        "description": "Example taxonomy database for bacterial phylogenomics",
        "n_genomes": 8,
        "n_taxa": 4,
        "taxonomic_levels": ["genus", "species"],
        "database_type": "OrthoPhyl"
    }
    ...
    print("Update taxonomy_map.tsv with your actual genomes")
...
Examples:
  ...
  python assembly_router.py \\
      --create-example-db example_db/

Input table format (for --batch):
  assembly_path<TAB>taxonomy<TAB>[assembly_id]
  
  Example:
  genome1.fna	Escherichia coli	ASM001
  genome2.fna	Klebsiella pneumoniae	ASM002
  genome3.fna	Novel_species	ASM003
        """
    )
    ...
    input_group.add_argument(
        '--assembly',
        help='Single assembly FASTA file'
    )
    input_group.add_argument(
        '--batch',
        help='TSV file with multiple assemblies (assembly_path, taxonomy, [id])'
    )
    input_group.add_argument(
        '--create-example-db',
        metavar='DIR',
        help='Create example database structure at DIR'
    )
    
    parser.add_argument(
        '--taxonomy',
        help='Taxonomic classification (for single assembly mode)'
    )
    parser.add_argument(
        '--assembly-id',
        help='Assembly identifier (optional, defaults to filename)'
    )
    
    # Database and output
    parser.add_argument(
        '--database-dir',
        required='--create-example-db' not in sys.argv,
        help='Path to reference taxonomy database directory'
    )
    parser.add_argument(
        '--output-dir',
    ...
    parser.add_argument(
        '--min-genomes-for-orthophyl',
        type=int,
        default=5,
        help='Min additional genomes to trigger OrthoPhyl (default: 5)'
    )
    parser.add_argument(
        '--ncbi-datasets',
        help='Path to ncbi-datasets CLI tool (default: datasets)'
    )
    ...
    parser.add_argument(
        '--execute',
        action='store_true',
        help='Execute the recommended pipeline (not just routing)'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    try:
        router = AssemblyRouter(
            ...
            ncbi_datasets_path=args.ncbi_datasets,
            threads=args.threads,
            min_genomes_for_orthophyl=args.min_genomes_for_orthophyl
        )
    except Exception as e:
    ...
    try:
        ...
        for decision in decisions:
            ...
            if args.execute and not args.dry_run:
                ...
                # subprocess.run(decision['command'], shell=True)
            else:
