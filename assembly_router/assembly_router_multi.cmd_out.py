...
class MultiDatabaseRouter:
    ...
    def __init__(
        ...
        for db in self.databases:
            logger.info(f"  - {db['clade_name']} ({db['rank_name']})")
    ...
    def find_matching_databases(self, query_taxonomy: str) -> List[Dict]:
        ...
        for db in self.databases:
            ...
            if query_tax.is_within_clade(clade_tax, clade_rank):
                matches.append({
                    'database': db,
                    'specificity': self.RANKS.index(clade_rank)  # Higher = more specific
                })
    ...
    def _route_to_orthophyl(
        ...
        # Step 1: Download related genomes
        if self.gather_filter_script and self.gather_filter_script.exists():
            gather_cmd = (
                f"# Step 1: Download related genomes from NCBI\n"
                f"{self.gather_filter_script} \\\n"
                f"    -t '{tax_string_for_download}' \\\n"
                f"    -o {orthophyl_input_dir} \\\n"
                f"    -n 50  # Adjust number of genomes as needed"
            )
        ...
        orthophyl_cmd = (
            ...
            f"./OrthoPhyl.sh \\\n"
            f"    -g {orthophyl_input_dir} \\\n"
            f"    -r {Path(assembly_path).stem} \\\n"
        ...
        db_cmd = (
            ...
            f"# Add to your orthophyl_runs.tsv:\n"
    ...
    def _generate_batch_summary(self, decisions: List[Dict]):
        ...
        with open(summary_file, 'w') as f:
            ...
            for db in self.databases:
                f.write(f"  - {db['clade_name']} ({db['rank_name']})\n")
            f.write("\n")
...
Examples:
  ...
  python assembly_router_multi.py \\
      ...
      --database-dir /path/to/databases/ \\
      --gather-filter-script util/gather_filter_asms.sh \\
      --output-dir results/
  ...
  python assembly_router_multi.py \\
    ...
    try:
        ...
        for decision in decisions:
            ...
            print(f"\n  Command saved to: routing_summary_{decision['assembly_id']}.txt")
